...
def block_toeplitz_par(file_name,n,b,pad):
	...
	lc=np.zeros(shape=(0,1), dtype=complex)
	lr=np.zeros(shape=(1,0), dtype=complex)
	...
	if rank==size-1 and pad==0:
		lc=g1[:,end-start-1:end-start]
		lr=g1[b-1:b,end-start-b:end-start]
	empty=0
	del a
	...
	for i in xrange(1,n+n*pad):    
		...
		if i>=n*pad and end_g1==global_end_g1 and length_g>0:
			print lc.shape, length_g, rank
			lc=np.concatenate((lc,g1[:,length_g-1:length_g]),axis=0)
			lr=np.concatenate((lr,g1[b-1:b,length_g-b:length_g]),axis=1)
			#print l.shape
	lc = comm.gather(lc, root=0)
	lr = comm.gather(lr, root=0)
	if rank==0:
		lc=np.concatenate(lc[::-1], axis=0)
		lr=np.concatenate(lr[::-1], axis=1)
	return lc,lr
