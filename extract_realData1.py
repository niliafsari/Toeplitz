...
path="processedData/gate0_numblock_%s_meff_%s_offsetn_%s_offsetm_%s/" %(str(sizen),str(meff*2),str(offsetn),str(offsetm))
mkdir="mkdir "+path
os.system(mkdir)
filen=path+"gate0_numblock_%s_meff_%s_offsetn_%s_offsetm_%s_dynamic.npy" %(str(sizen),str(meff*2),str(offsetn),str(offsetm))
...
for j in xrange(0,neff/2):
	if j:
		...
		input_f[0:meff_f,j*meff_f:(j+1)*meff_f]=toeplitz(cols,rows)
	else:
		input_f[0:meff_f,j*meff_f:(j+1)*meff_f]=toeplitz(np.conj(np.append(a_input[j,:meff-const],np.zeros(pad2*meff+const))))+epsilon
...
for rank in xrange(0,nump):
	...
	np.save(file_name,input_f[:,start:start+size_node])
